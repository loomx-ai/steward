...
func (r *Runtime) listBillingBudgets(ctx context.Context, c *client, request contracts.InventoryRequest) (contracts.InventoryBatch, error) {
	...
	values, err := c.visibleBillingBudgets(ctx)
	if err != nil {
		return failed, err
	}
	...
	for _, id := range ids {
		...
		// Keep account failure distinct from the saved budget's own absence.
		account, err := c.billingRead(ctx, "cloudbilling.billingAccounts.get", parent)
		if err != nil {
			return failed, err
		}
		if err := billingAccountData(account); err != nil {
			return failed, err
		}
		...
		again, err := c.billingRead(ctx, "cloudbilling.billingAccounts.get", parent)
		if err != nil {
			return failed, err
		}
		if err := billingAccountData(again); err != nil {
			return failed, err
		}
