...
// scan worker. Losing access to parent jobs cannot close their observed tasks.
func TestBatchTaskScanPermissionFailurePreservesObservations(t *testing.T) {
	ctx := context.Background()
	s := newBatchScenario(t)
	s.emptyPage = true
	r := protocolRuntime(t, s.transport(t))
	kind := r.resourceKind(batchTaskType)
	...
	repositories, err := sqlite.Open(filepath.Join(t.TempDir(), "batch-inventory.db"), filepath.Join("..", "..", "migrations"))
	if err != nil {
		t.Fatal(err)
	}
	now := time.Date(2026, 9, 9, 0, 0, 0, 0, time.UTC)
	connection := asset.CloudConnection{ID: "connection", Name: "Batch", Provider: asset.ProviderGCP, Partition: "google-cloud", Principal: "fixture", Status: asset.ConnectionActive, CreatedAt: now, UpdatedAt: now}
	scope := asset.Scope{ID: "batch-region", ConnectionID: connection.ID, Kind: asset.ScopeRegion, NativeID: "us-central1", Name: "us-central1", CreatedAt: now, UpdatedAt: now}
	...
	for _, runID := range []string{"first", "denied"} {
		...
		shard := asset.ScanShard{ID: asset.ScanShardID("batch-" + runID), ScanRunID: run.ID, Provider: asset.ProviderGCP, Source: source.Name, ScopeID: scope.ID, ResourceKindID: kind.ID, Authoritative: source.AuthoritativeDefault, Status: asset.ShardPending, CreatedAt: now}
		...
		if runID == "denied" {
			s.handle = func(req *http.Request) (*http.Response, bool) {
				if req.URL.Path == "/v1/projects/sample-project/locations/us-central1/jobs" {
					return dataformResponse(req, 403, map[string]any{}), true
				}
				return nil, false
			}
		}
		err := handler.Handle(ctx, execution.Job{ID: execution.JobID("batch-" + runID), Type: execution.JobScan, Payload: map[string]any{"scan_shard_id": string(shard.ID)}})
		...
		if err != nil || len(page.Items) != 2 {
			t.Fatalf("Task projection: %d %v", len(page.Items), err)
		}
		for _, value := range page.Items {
			if value.ClosedAt != nil || value.DeletedAt != nil || text(value.Normalized[batchParentProof]) == "" || text(value.Normalized["_batch_job_uid"]) == "" {
				t.Fatalf("failed scan lost native task proof: %+v", value)
			}
		}
		encoded, _ := json.Marshal(page)
		if strings.Contains(string(encoded), "BATCH_PRIVATE_") {
			t.Fatal("task payload escaped SQLite redaction")
		}
