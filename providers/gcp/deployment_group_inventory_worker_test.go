...
// A failed group-revision read must preserve all previously observed revision records.
func TestDeploymentGroupScanFailurePreservesRevisionRecords(t *testing.T) {
	ctx := context.Background()
	s := newDeploymentGroupScenario(t)
	r := protocolRuntime(t, s.transport(t))
	kind := r.resourceKind(infraGroupRevision)
	...
	repositories, err := sqlite.Open(filepath.Join(t.TempDir(), "deployment-group-inventory.db"), filepath.Join("..", "..", "migrations"))
	if err != nil {
		t.Fatal(err)
	}
	now := time.Date(2026, 9, 9, 0, 0, 0, 0, time.UTC)
	connection := asset.CloudConnection{ID: "connection", Name: "Deployment group", Provider: asset.ProviderGCP, Partition: "google-cloud", Principal: "fixture", Status: asset.ConnectionActive, CreatedAt: now, UpdatedAt: now}
	scope := asset.Scope{ID: "deployment-group-global", ConnectionID: connection.ID, Kind: asset.ScopeRegion, NativeID: "us-central1", Name: "us-central1", CreatedAt: now, UpdatedAt: now}
	...
	for _, runID := range []string{"first", "denied"} {
		...
		shard := asset.ScanShard{ID: asset.ScanShardID("deployment-group-" + runID), ScanRunID: run.ID, Provider: asset.ProviderGCP, Source: source.Name, ScopeID: scope.ID, ResourceKindID: kind.ID, Authoritative: source.AuthoritativeDefault, Status: asset.ShardPending, CreatedAt: now}
		...
		if runID == "denied" {
			s.hook = func(req *http.Request) (*http.Response, bool) {
				if req.URL.Path == "/v1/"+groupTestName+"/revisions/g-2" {
					return dataformResponse(req, 403, map[string]any{"error": map[string]any{"code": 403, "message": "GROUP_PRIVATE_IAM_FAILURE"}}), true
				}
				return nil, false
			}
		}
		err := handler.Handle(ctx, execution.Job{ID: execution.JobID("deployment-group-" + runID), Type: execution.JobScan, Payload: map[string]any{"scan_shard_id": string(shard.ID)}})
		...
		if strings.Contains(string(encoded), "GROUP_PRIVATE_") {
			t.Fatal("Deployment group IAM error escaped SQLite redaction")
		}
