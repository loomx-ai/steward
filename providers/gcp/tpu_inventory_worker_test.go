...
// Queue or disk reads are part of authoritative Node inventory. A permission
// failure must preserve previously observed Nodes and their frozen disk proofs.
func TestTPUNodeScanPermissionFailurePreservesObservations(t *testing.T) {
	ctx := context.Background()
	s := newTPUScenario(t)
	s.emptyPage = true
	r := protocolRuntime(t, s.transport(t))
	kind := r.resourceKind(tpuNodeType)
	...
	repositories, err := sqlite.Open(filepath.Join(t.TempDir(), "tpu-inventory.db"), filepath.Join("..", "..", "migrations"))
	if err != nil {
		t.Fatal(err)
	}
	now := time.Date(2026, 9, 9, 0, 0, 0, 0, time.UTC)
	connection := asset.CloudConnection{ID: "connection", Name: "Cloud TPU", Provider: asset.ProviderGCP, Partition: "google-cloud", Principal: "fixture", Status: asset.ConnectionActive, CreatedAt: now, UpdatedAt: now}
	scope := asset.Scope{ID: "tpu-region", ConnectionID: connection.ID, Kind: asset.ScopeRegion, NativeID: "us-central2", Name: "us-central2", CreatedAt: now, UpdatedAt: now}
	...
	for _, runID := range []string{"first", "denied"} {
		...
		shard := asset.ScanShard{ID: asset.ScanShardID("tpu-" + runID), ScanRunID: run.ID, Provider: asset.ProviderGCP, Source: source.Name, ScopeID: scope.ID, ResourceKindID: kind.ID, Authoritative: source.AuthoritativeDefault, Status: asset.ShardPending, CreatedAt: now}
		...
		if runID == "denied" {
			s.handle = func(req *http.Request) (*http.Response, bool) {
				if req.URL.Path == "/v2/"+tpuTestQueue {
					return dataformResponse(req, 403, map[string]any{}), true
				}
				return nil, false
			}
		}
		err := handler.Handle(ctx, execution.Job{ID: execution.JobID("tpu-" + runID), Type: execution.JobScan, Payload: map[string]any{"scan_shard_id": string(shard.ID)}})
		...
		if err != nil || len(page.Items) != 2 {
			t.Fatalf("Node projection: %d %v", len(page.Items), err)
		}
		for _, value := range page.Items {
			if value.ClosedAt != nil || value.DeletedAt != nil || text(value.Normalized[tpuProof]) == "" || text(value.Normalized[tpuDiskProofs]) == "" || text(value.Normalized[tpuQueueProof]) == "" {
				t.Fatalf("failed scan lost native Node proof: %+v", value)
			}
		}
		encoded, _ := json.Marshal(page)
		if strings.Contains(string(encoded), "TPU_PRIVATE_") {
			t.Fatal("Node metadata escaped SQLite redaction")
		}
