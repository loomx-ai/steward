...
func loadProviderData() (providerMetadata, error) {
	...
	for _, kind := range result.kinds {
		...
		if (definition.Discovery.Source == productInventorySource || definition.Discovery.Source == dataformInventorySource || definition.Discovery.Source == firewallInventorySource || definition.Discovery.Source == organizationInventorySource) && (definition.Discovery.List == nil || !slices.Contains(kind.ListOperations, definition.Discovery.List.Operation)) {
