...
func (c *client) workbookIndex(ctx context.Context, kind string, known []string, groups map[string]map[string]any) (map[string]map[string]any, string, error) {
	...
	if kind != insightsWorkbookTemplateType {
		...
		for _, id := range known {
			...
			if err != nil || canonical != id || insightsWorkbookKind(typ) != kind || !strings.HasPrefix(id, c.root()+"/") || seedIDs[id] {
				return nil, "", serviceDenied("invalid_known_workbook_identity")
			}
			seedIDs[id] = true
		}
		...
		for _, value := range rows {
			...
			if err != nil || !strings.HasPrefix(id, c.root()+"/") || seen[id] {
				return nil, "", serviceDenied("invalid_workbook_arm_index")
			}
			seen[id] = true
			...
			if !validResponseType(kind, text(raw["type"])) {
				return nil, "", serviceDenied("workbook_arm_index_type_changed")
			}
			seedIDs[id] = true
		}
		for _, id := range slices.Sorted(maps.Keys(seedIDs)) {
			...
			if isNotFound(err) {
				if seen[id] {
					return nil, "", serviceDenied("workbook_arm_index_changed")
				}
				continue // Bounded-source omissions do not close any saved asset.
			}
			if err != nil {
				return nil, "", err
			}
...
func (r *Runtime) workbookInventorySnapshot(ctx context.Context, c *client, request contracts.InventoryRequest) ([]contracts.InventoryItem, string, error) {
	kind := insightsWorkbookKind(request.ResourceKind.NativeType)
	groups, err := c.insightsGroups(ctx)
	if err != nil {
		return nil, "", err
	}
	values, requestID, err := c.workbookIndex(ctx, kind, request.KnownNativeIDs, groups)
	if err != nil {
		return nil, "", err
	}
...
func (r *Runtime) listInsightsWorkbooks(ctx context.Context, c *client, request contracts.InventoryRequest) (batch contracts.InventoryBatch, err error) {
	...
	first, provenance, err := r.workbookInventorySnapshot(ctx, c, request)
	if err != nil {
		return batch, err
	}
	second, _, err := r.workbookInventorySnapshot(ctx, c, request)
	if err != nil {
		return batch, err
	}
	...
	if c.privateConfiguration(bindings(first)) != c.privateConfiguration(bindings(second)) {
		return batch, serviceDenied("workbook_collection_changed_during_scan")
	}
	boundary := request
	boundary.Cursor, boundary.Limit = "", 0
	fingerprint := c.privateConfiguration(map[string]any{"request": boundary, "revision": r.bundle.Revision, "bindings": bindings(first)})
