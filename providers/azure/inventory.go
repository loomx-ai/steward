...
func (r *Runtime) List(ctx context.Context, request contracts.InventoryRequest) (contracts.InventoryBatch, error) {
	...
	if request.Source == dataProtectionSource && !protection || protection && request.Source != "" && request.Source != inventorySource && request.Source != dataProtectionSource {
		return contracts.InventoryBatch{}, serviceDenied("invalid_data_protection_source")
	}
	stack := request.ResourceKind != nil && strings.EqualFold(request.ResourceKind.NativeType, deploymentStackType)
	if request.Source == deploymentStackSource && !stack || stack && request.Source != "" && request.Source != inventorySource && request.Source != deploymentStackSource {
		return contracts.InventoryBatch{}, serviceDenied("invalid_deployment_stack_source")
	}
	netapp := request.ResourceKind != nil && netappKind(request.ResourceKind.NativeType).kind != ""
	if request.Source == netappSource && !netapp || netapp && request.Source != "" && request.Source != inventorySource && request.Source != netappSource {
		return contracts.InventoryBatch{}, serviceDenied("invalid_netapp_source")
	}
	backup := request.ResourceKind != nil && synapseBackupKind(request.ResourceKind.NativeType) != ""
	if request.Source == synapseBackupSource && !backup || backup && request.Source != "" && request.Source != inventorySource && request.Source != synapseBackupSource {
		return contracts.InventoryBatch{}, serviceDenied("invalid_synapse_backup_source")
	}
	...
	if request.ResourceKind != nil {
		_, directory = graphKindOf(request.ResourceKind.NativeType)
	}
	if request.Source == graphDirectorySource && !directory || directory && request.Source != graphDirectorySource {
		return contracts.InventoryBatch{}, serviceDenied("invalid_graph_directory_source")
	}
	...
	if protection {
		...
		request.Source = dataProtectionSource
		return r.listDataProtection(ctx, c, request)
	}
	if stack {
		if request.Source == inventorySource {
			return contracts.InventoryBatch{Complete: true}, nil
		}
		request.Source = deploymentStackSource
		return r.listDeploymentStacks(ctx, c, request)
	}
	if netapp {
		if request.Source == inventorySource {
			return contracts.InventoryBatch{Complete: true}, nil
		}
		request.Source = netappSource
		return r.listNetapp(ctx, c, request)
	}
	if backup {
		if request.Source == inventorySource {
			return contracts.InventoryBatch{Complete: true}, nil
		}
		request.Source = synapseBackupSource
		return r.listSynapseBackups(ctx, c, request)
	}
	if synapseData {
		if request.Source == inventorySource {
			return contracts.InventoryBatch{Complete: true}, nil
	...
	if directory {
		return r.listGraphDirectory(ctx, c, request)
	}
	if defender {
		if request.Source == inventorySource {
			return contracts.InventoryBatch{Complete: true}, nil
