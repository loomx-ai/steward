...
func (r *Runtime) ResolveAction(ctx context.Context, id asset.ConnectionID, value asset.Asset) (resolved contracts.ActionDriver, err error) {
	...
	if kind.NativeType == recoveryServicesItem {
		return newRecoveryItemAction(c, id, value)
	}
	if kind.NativeType == recoveryServicesContainer {
		return newRecoveryContainerAction(c, id, value)
	}
	if kind.NativeType == dataProtectionVault {
		return newProtectionVaultAction(c, id, value)
	}
	if kind.NativeType == dataProtectionInstance {
		return newProtectionInstanceAction(c, id, value)
	}
	if kind.NativeType == dataProtectionPolicy {
		return newProtectionPolicyAction(c, id, value)
	}
