...
func TestElasticSanInventoryNativePopulationsAndSQLite(t *testing.T) {
	...
	for _, kind := range kinds {
		...
		for _, item := range batch.Items {
			if item.Location != "eastus" || item.Actionable == nil || *item.Actionable != (elasticSanIndependentChild(kind) && kind != elasticSanType && !(kind == elasticSanGroupType && f.retained[item.NativeID])) || item.Normalized["retained"] != f.retained[item.NativeID] {
			...
			if _, err := f.runtime.ResolveAction(ctx, "connection", elasticSanTestAsset(item)); (err == nil) != (elasticSanIndependentChild(kind) && kind != elasticSanType && !(kind == elasticSanGroupType && f.retained[item.NativeID])) {
