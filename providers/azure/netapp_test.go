...
func TestNetappNativeInventory(t *testing.T) {
	for _, kind := range netappResources {
		t.Run(kind.family, func(t *testing.T) {
			...
			if strings.Contains(string(wire), "netapp-private-canary") || first.Actionable == nil || *first.Actionable != (kind.kind == netappSnapshotPolicyType || kind.kind == netappPoolType || kind.kind == netappVolumeType || netappDirectLeaf(kind.kind)) {
