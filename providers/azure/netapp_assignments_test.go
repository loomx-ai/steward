...
func TestNetappAssignmentInventoryAndKnownConsumers(t *testing.T) {
	for _, field := range netappAssignmentFields {
		t.Run(field.kind, func(t *testing.T) {
			...
			if strings.Contains(string(wire), "netapp-private-canary") || item.Actionable == nil || *item.Actionable != (field.kind == netappSnapshotPolicyType || field.kind == netappVaultType) {
