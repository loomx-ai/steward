...
func safeAPIPayload(value map[string]any, endpoint string) map[string]any {
	...
	if err == nil && u.Host == "management.azure.com" && armPathProvider(u.Path) == "microsoft.recoveryservices" {
		return safePayload(object(recoveryServicesSafeValue(value)))
	}
	...
	if err == nil && u.Host == "management.azure.com" && armPathProvider(u.Path) == "microsoft.netapp" {
		return safePayload(object(netappSafeValue(value)))
	}
	if err == nil && u.Host == "management.azure.com" && armPathProvider(u.Path) == "microsoft.synapse" && (strings.Contains(strings.ToLower(u.Path), "/operationstatuses/") || strings.Contains(strings.ToLower(u.Path), "/operationresults/")) {
		return safePayload(object(synapseOperationSafeValue(value)))
	}
	if err == nil && strings.HasSuffix(u.Host, ".dev.azuresynapse.net") {
		return safePayload(object(synapseDataSafeValue(value)))
	}
	...
	if err == nil && (u.Host == "management.azure.com" && armPathProvider(u.Path) == "microsoft.communication" || communicationEndpointPattern.MatchString(u.Scheme+"://"+u.Host)) {
		cleaned := object(communicationSafeValue(value))
	...
	if err == nil && u.Host == "management.azure.com" {
		...
		if fleetPath(u.Path) {
			cleaned := object(fleetSafeValue(value))
			if value["path"] == u.Path && (value["method"] == "GET" || value["method"] == "DELETE" || value["method"] == "POST") {
				cleaned["method"], cleaned["path"] = value["method"], u.Path
				cleaned["query"] = map[string]any{"api-version": u.Query().Get("api-version")}
			}
			value = cleaned
