	...
	for _, kind := range result.kinds {
		...
		for _, ids := range [][]string{kind.ReadOperations, kind.DeleteOperations, kind.ListOperations} {
			for _, id := range ids {
				...
				if !ok || operation.Call == nil || (operation.Call.Style != "azure-rest" && operation.Call.Style != "azure-batch-rest" && operation.Call.Style != "azure-communication-rest" && operation.Call.Style != "azure-synapse-rest" && operation.Call.Style != "azure-keyvault-rest") {
