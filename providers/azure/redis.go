...
func (a *action) resourceOperationResponse(endpoint string, res response) error {
	if !isRedisType(a.kind.NativeType) && !isSearchType(a.kind.NativeType) && !isCognitiveType(a.kind.NativeType) && !isCosmosType(a.kind.NativeType) && !isMongoClusterType(a.kind.NativeType) && !isKustoType(a.kind.NativeType) {
		return nil
	}
