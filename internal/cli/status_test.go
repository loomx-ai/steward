...
func TestServerStartExposesOnlyTerminalRuntimeConfiguration(t *testing.T) {
	cmd := newServerStartCommand("dev")
	for _, forbidden := range []string{"executor", "scan-concurrency"} {
		if flag := cmd.Flags().Lookup(forbidden); flag != nil {
			t.Fatalf("forbidden flag still exposed: %s", flag.Name)
		}
	}
	if flag := cmd.Flags().Lookup("auth-token"); flag == nil {
		t.Fatal("auth-token flag is missing")
	}
	driver := cmd.Flags().Lookup("db-driver")
	if driver == nil || !strings.Contains(driver.Usage, "sqlite or postgres") || strings.Contains(driver.Usage, "mysql") || strings.Contains(driver.Usage, "memory") {
		t.Fatalf("db-driver usage = %#v", driver)
	}
}
