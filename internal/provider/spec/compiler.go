...
func validateOperationCall(operation catalog.Operation, usage string) error {
	...
	if call.Style != "RPC" && call.Style != "ROA" && call.Style != "google-rest" && call.Style != "azure-rest" && call.Style != "azure-batch-rest" {
...
func validateParameterExpressions(
	...
	for parameter, value := range parameters {
		...
		switch text {
		case "scope.location", "scope.project", "scope.projectPath", "scope.metricsScopeName", "scope.locationParent", "scope.regionParent", "scope.allLocationsParent", "scope.iapTunnelLocationParent", "scope.subscription":
