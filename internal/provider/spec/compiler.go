...
func validateOperationCall(operation catalog.Operation, usage string) error {
	...
	if call.Style != "RPC" && call.Style != "ROA" && call.Style != "google-rest" && call.Style != "azure-rest" && call.Style != "azure-batch-rest" && call.Style != "azure-communication-rest" && call.Style != "azure-synapse-rest" && call.Style != "azure-keyvault-rest" && call.Style != "azure-graph-rest" && call.Style != "aws-smithy" {
...
func validateParameterExpressions(
	...
	for parameter, value := range parameters {
		...
		switch text {
		case "scope.location", "scope.project", "scope.projectPath", "scope.metricsScopeName", "scope.securityBillingName", "scope.locationParent", "scope.regionParent", "scope.allLocationsParent", "scope.iapTunnelLocationParent", "scope.subscription", "scope.subscriptionPath", "scope.accountId", "scope.wafScope":
