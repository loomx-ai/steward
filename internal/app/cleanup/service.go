...
func requiresTypedName(selector plan.CleanupSelector) bool {
	return selector.Kind == plan.SelectorConnection ||
		(selector.Kind == plan.SelectorScope && (selector.ScopeKind == asset.ScopeAccount || selector.ScopeKind == asset.ScopeProject || selector.ScopeKind == asset.ScopeSubscription || selector.ScopeKind == asset.ScopeRegion))
}
...
func appendSelectionWarnings(values []plan.Warning, input plan.Input, solved plan.Result) []plan.Warning {
	...
	for _, value := range input.Assets {
		...
		case "Microsoft.RecoveryServices/vaults/backupFabrics/protectionContainers/protectedItems":
			if value.Identity.Provider == asset.ProviderAzure {
				result = append(result, plan.Warning{Code: plan.WarningRecoveryServicesItemDelete, AssetID: value.ID, Message: "Deleting this backup item stops its backups and requests deletion of recovery points under Azure retention rules. Soft-deleted backups may remain recoverable and incur charges; completion does not mean permanent purge. The source workload, policy, protection container and vault are retained.", Evidence: map[string]any{"operation": "delete", "native_type": value.Identity.NativeType}})
			}
		...
		case "Microsoft.NetApp/netAppAccounts/backupVaults":
			if value.Identity.Provider == asset.ProviderAzure && !selectionWarningExists(result, plan.WarningNetappVaultDelete, value.ID) {
				result = append(result, plan.Warning{Code: plan.WarningNetappVaultDelete, AssetID: value.ID, Message: "Deleting this backup vault stops scheduled backups on its reviewed volumes, removes their backup assignments, and permanently deletes all reviewed backups in the vault. Removing the final backup also removes the reference point for future incremental backups. The volumes, snapshots, subvolumes, quota rules and backup policies are retained.", Evidence: map[string]any{"operation": "delete", "native_type": value.Identity.NativeType}})
			}
