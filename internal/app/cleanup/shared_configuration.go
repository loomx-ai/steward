...
func settleSharedConfiguration(ctx context.Context, repositories persistence.Repositories, task persistence.CleanupTaskAggregate, attempt execution.ExecutionAttempt, step plan.CleanupTaskStep, registry ProviderActionRegistry) (bool, error) {
	...
	if reviewed.Identity.NativeType == "monitoring.googleapis.com/UptimeCheckConfig" || reviewed.Identity.NativeType == "compute.googleapis.com/Router" || reviewed.Identity.NativeType == "monitoring.googleapis.com/NotificationChannel" || reviewed.Identity.NativeType == "monitoring.googleapis.com/Group" || reviewed.Identity.NativeType == "monitoring.googleapis.com/AlertPolicy" {
