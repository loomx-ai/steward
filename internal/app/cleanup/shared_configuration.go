...
func settleSharedConfiguration(ctx context.Context, repositories persistence.Repositories, task persistence.CleanupTaskAggregate, attempt execution.ExecutionAttempt, step plan.CleanupTaskStep, registry ProviderActionRegistry) (bool, error) {
	...
	if reviewed.Identity.NativeType != "compute.googleapis.com/Router" && reviewed.Identity.NativeType != "compute.googleapis.com/RouterNat" && reviewed.Identity.NativeType != "compute.googleapis.com/RoutePolicy" && reviewed.Identity.NativeType != "compute.googleapis.com/NamedSet" && reviewed.Identity.NativeType != "monitoring.googleapis.com/AlertPolicy" && reviewed.Identity.NativeType != "monitoring.googleapis.com/NotificationChannel" {
		return false, nil
	}
