...
type CloudProvider = "alicloud" | "aws" | "gcp" | "azure";
...
export function CloudProviderIcon({
  ...
  const cloudProvider = resolveCloudProvider(provider, consoleURL);
  const sharedProps = {
  ...
  if (cloudProvider === "alicloud") {
    return (
      <svg
        {...props}
        {...sharedProps}
        data-cloud-provider="alicloud"
        fill="currentColor"
        viewBox="0 0 24 24"
        className={cn("text-provider-alicloud", sharedProps.className)}
      >
  ...
  if (cloudProvider === "aws") {
    return (
      <svg
        ...
        fill="currentColor"
        viewBox="0 0 24 24"
        className={cn("text-provider-aws", sharedProps.className)}
      >
  ...
  if (cloudProvider === "azure") {
    return <Cloud {...props} {...sharedProps} data-cloud-provider="azure" className={cn("text-provider-azure", sharedProps.className)} />;
  }
  ...
  return (
    <Cloud
      {...props}
      {...sharedProps}
      data-cloud-provider="unknown"
      className={cn("text-muted-foreground", sharedProps.className)}
    />
  );
}
...
function resolveCloudProvider(
  ...
  if (
    ...
    normalizedProvider === "alibaba cloud"
  ) {
    return "alicloud";
  }
  if (
    normalizedProvider === "aws" ||
    normalizedProvider === "amazon web services"
  ) {
    return "aws";
  }
  ...
  if (normalizedProvider === "azure" || normalizedProvider === "microsoft azure") return "azure";
  ...
  try {
    ...
    if (hostname === "portal.azure.com") return "azure";
    if (
      hostname === "aliyun.com" ||
    ...
    if (
      ...
      hostname === "console.amazonaws.cn" ||
      hostname.endsWith(".console.amazonaws.cn")
    ) {
      return "aws";
    }
  } catch {
