...
Steward 识别 203 类阿里云资源，其中 177 类具有原生清理操作；资源中心返回的其他类型作为只读清单展示。所有产品 API 调用都锁定到 api.aliyun.com 发布的官方元数据，列表与读取的响应路径逐一对照官方响应结构和示例验证。覆盖范围仍在扩展，尚未完整覆盖阿里云的所有产品。

常用资源包括 ECS 实例、磁盘和网卡，VPC、交换机、安全组、NAT 网关、弹性公网 IP，ALB、NLB、CLB 及其监听和服务器组，RDS、Redis、PolarDB，Kafka、RocketMQ、RabbitMQ 及其 Topic 和消费组，OSS、日志服务、容器镜像，云监控、配置审计、SAE、EMR、无影云电脑和事件总线等。实际可发现的范围取决于资源类型支持、所选地域和当前身份的权限。
...
- **负载均衡子资源：** 删除 ALB、NLB 或 CLB 实例时，计划先逐项删除其监听和 CLB 虚拟服务器组。服务器组、CLB 访问控制策略组和证书是独立资源：仍被监听或转发规则使用时，云端会拒绝删除；由 ALB Ingress 管理的服务器组不会直接删除。
- **消息队列 Topic 与分组：** 删除 Kafka、RocketMQ 4.0 或 5.0 实例时，计划先逐项删除实例中的 Topic 和消费组，并在实例仍可查询时确认各自已删除。RocketMQ 4.0 只删除本账号持有的 Topic，其他账号授权的 Topic 不会删除。删除 Topic 会丢弃其中的消息。
- **镜像仓库、日志库与 VPN 服务端：** 删除容器镜像命名空间会一并删除其中的仓库和镜像，计划先逐个删除仓库。删除日志项目会删除全部日志库（包括服务创建的 `internal-` 日志库），计划将其列为随项目删除，并在项目删除后逐一确认。SSL 客户端证书、SSL 服务端和 IPsec 服务端先于所属 VPN 网关删除。
- **云监控：** 可清理自建应用分组、报警规则、报警联系人和联系组。由其他服务、标签或资源组同步生成的应用分组会被来源重新创建，不会直接删除。删除联系人或联系组后，引用它们的报警规则将不再通知这些对象。
- **SAE：** 应用先于所在命名空间删除；各地域的默认命名空间不会删除。应用删除为异步操作，最多等待 10 分钟确认完成。
- **配置审计：** 规则与合规包在 cn-shanghai 和 ap-southeast-1 盘点。删除合规包会一并删除它创建的规则，计划在合规包删除后逐一确认；这些规则不能单独删除。账号组仅盘点。
- **无影云电脑：** 仅清理按量付费、且不在云电脑池中的云电脑；包年包月云电脑到期自动释放。办公网络须在其中云电脑全部释放后删除；系统策略不会删除。
- **事件总线与 RabbitMQ：** 事件规则先于所属事件总线删除。RabbitMQ 实例仅删除后付费实例，预付费实例到期自动释放。
- **EMR 集群：** 仅清理按量付费集群。开启释放保护的集群会先关闭保护再删除；集群状态变为 TERMINATED 即视为完成，最多等待 10 分钟。
