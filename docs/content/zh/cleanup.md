...
即使直接选择了目标，某些依赖也会阻塞任务：未选中的资源仍在使用的网络、密钥或身份（例如 AWS KMS 密钥或 IAM 角色、Google Cloud KMS 密钥或服务账号、Azure 托管身份）。请将依赖资源加入任务，或保留该目标。删除此类密钥或身份还要求连接的所有活跃地域与全局范围均已完整扫描，因为未扫描的资源可能仍在使用它。

连接自身使用的身份始终受保护：连接背后的 AWS IAM 用户或角色及其用户组、附加的托管策略和实例配置文件；阿里云 RAM 用户或角色、用户所在的用户组，以及直接或经用户组附加到它们的自定义权限策略；Google Cloud 服务账号及其密钥；以及授予连接服务主体的 Azure 角色分配。删除它们会在清理中途撤销 Steward 的访问权限。

选中的控制器无法清理时，依赖它的子资源删除步骤也会被阻塞。如需独立清理受支持的子资源，请仅选择这些子资源并重新审查任务。
