...
Steward 识别 491 类资源，其中 440 类具有原生清理操作（包括 Batch 节点移除），执行时受下列条件约束。ARM 返回的其他资源类型作为只读清单展示。覆盖范围仍在扩展，尚未完整覆盖 Azure 的所有产品。
...
| Azure Synapse | 工作区、Spark 池、SQL 池、Spark 任务与会话、Notebook 和 Spark 作业定义 | 支持盘点、工作区、池及默认 Data Lake 引用；清理尚未实现 |
...
| Microsoft Entra 用户与组 | 通过 Microsoft Graph（独立的 `graph.microsoft.com` 令牌）读取的用户和组，含组的直接成员 | 只读：目录对象影响整个租户。需要经管理员同意的 Microsoft Graph 应用权限 `User.Read.All` 与 `GroupMember.Read.All`（或 `Directory.Read.All`）；仅保存显示名称、用户主体名称、账户状态、用户类型、组类型标记与创建时间 |
...
Synapse 使用原生列表和详情读取。列表遗漏不会移除已知资源，只有该资源自身的 GET 确认不存在后才会关闭旧记录。默认 Data Lake 存储作为独立依赖保留。数据平面接口现支持读取 Spark 任务与会话、Notebook 和 Spark 作业定义，并校验工作区归属、使用独立认证 audience。返回数据和诊断日志不包含代码、任务配置和运行日志。这四类数据平面对象也已登记为盘点资产，并关联工作区和池。扫描校验原生分页，保留列表遗漏的已知对象。原生 API 还支持取消 Spark 任务和会话，并检查保护状态、读取详情确认结果。取消后可能保留已停止的历史记录，不能据此判断资源已删除。调用需要 Synapse 数据平面取消权限，以及工作区、池、资源组和管理锁的 ARM 读取权限。ARM 异步状态和结果读取也会校验操作范围，并区分操作完成与资源不存在；这需要工作区操作端点的读取权限。经审查的清理流程仍在实现中，这些资产暂不提供清理操作。
...
- **网络占用**：删除前立即检查：子网不能有 IP 配置、专用终结点、服务关联链接或资源导航链接、应用程序网关 IP 配置或 IP 配置文件；网络安全组、路由表或 NAT 网关不能关联子网或网络接口；公共 IP 地址不能附加到 IP 配置或 NAT 网关。同一任务中先删除的占用方可满足该检查；服务关联链接须由其所属服务移除。仅有委派不会阻止子网删除。
...
Azure Local 磁盘和网卡支持在全部原生 VM 引用解除后独立清理。需要 `Microsoft.AzureStackHCI/virtualHardDisks/delete` 或 `Microsoft.AzureStackHCI/networkInterfaces/delete`、对应资源读取、订阅范围的 Arc 机器列表/读取和 Local VM 列表/读取，以及资源组和管理锁读取权限。扫描保留已验证的 VM 身份，以恢复父索引遗漏；内部恢复记录不作为网络成员关系。仍在使用该资源的 VM 必须明确选中并先删除，或通过原生管理工具解除挂接后重新扫描。仅选择磁盘或网卡不会自动选择 VM 删除。系统盘继续作为 VM 的受管删除影响。保护、配置变化、读取不可用或新增引用会阻止清理。重启恢复、同步 204 或 DELETE 404 后，仍须确认资源自身不存在且 VM 引用已解除，才能完成。删除磁盘可能永久移除数据。参见原生[磁盘删除](https://learn.microsoft.com/en-us/rest/api/stackhci/virtual-hard-disks/delete?view=rest-stackhci-2024-01-01)和[网卡删除](https://learn.microsoft.com/en-us/rest/api/stackhci/network-interfaces/delete?view=rest-stackhci-2024-01-01)契约。物理资源移除及计费结果仍未验证。
...
Elastic SAN 盘点覆盖 SAN、卷组、卷、快照与私有终结点连接，使用 `2026-04-01-preview` API，分别读取活动和软删除卷／卷组列表。保留中的资源仍会显示，并保留其原生 ID；恢复卷时可能改变 ID，同时保留其 `volumeId`。活动列表为空不能证明永久移除。

Elastic SAN 的公开读取会在返回结果前核验资源身份、记录格式和分页。外部资源、格式错误的页面、非终态响应及不安全的续页链接会导致调用失败；原生请求 ID、ARM 标识的大小写兼容及显式活动／保留列表选择保持不变。

SAN 级盘点也会读取活动及保留卷组下的卷、快照和私有连接集合，保存签名成员范围，并在列表遗漏时通过已知子资源的自身读取恢复记录。即使只扫描 SAN，也需要这些子资源的列表／读取权限。保留卷组的快照列表返回 404 时，成员范围会标为不完整，仍可通过自身读取确认的已知快照继续保留；权限失败或卷集合缺失仍使扫描失败。

SAN 或卷组在外部删除后，盘点仍会尝试读取活动与保留两类列表，并逐个读取已知资源。只有确认父资源自身不存在后，才允许子集合缺失；仍存活的子资源会保留已记录的区域和网络关联。此前只能通过保留列表读取的资源，在该列表不可用时不能被判为不存在。只扫描子资源时，未扫描的父记录保持打开；原生成员信息过期或缺失会保存为清理约束，需要完整补扫相关资源后对账。SAN 的创建身份或可写配置变化，也要求重新扫描和审查后才能清理子资源。

盘点源会核对两次完整快照，恢复索引遗漏的已知 ID，并在权限或集合不完整时保留已有观测。子资源继承已验证的 SAN 区域。父资源缺失时，只有签名验证通过的历史观测才能提供区域与网络信息，而且子资源集合必须仍然可读。父资源、子网、源卷、私有终结点和控制器引用会进入关系图；控制器引用不授予删除所有权。

卷组盘点还会读取活动／保留卷、快照以及 SAN 私有终结点连接，完整核验成员需要这些子资源的列表／读取权限。核验通过的卷组显示成员数量；无法解析卷组映射的私有连接仍作为可能的依赖保留。签名历史会恢复列表遗漏的已知成员。保留卷组只保留历史成员信息，不将其显示为当前数量；创建中的卷尚无 GUID 时，成员信息也保持未核验。如果保留卷组的快照列表返回 404，五类资源仍可完成扫描与关系图重建：仍可发现的卷继续显示但禁止清理，已知快照则逐个读取。已知快照自身返回 404 可以完成其缺失对账，列表不可用不能证明未知快照不存在。卷成员不完整会保存为清理约束，直到重新扫描确认集合可读；已知快照仍可独立选中。权限失败或活动卷组的快照列表缺失仍会使扫描失败。
...
已支持活动卷组清理。计划会先清理活动卷及其快照，再清理卷组中其余快照，最后删除卷组。私有终结点连接需要独立选中并先确认不存在；其使用方 Network 私有终结点仍独立管理。已有保留卷会列为保留影响，完成清理时必须仍然存在。含有保留卷的卷组需要核验为 Enabled 的保留策略；暂不支持单独永久清除保留卷组。卷组 DELETE 不包含强制删除或永久删除选项，此流程不会断开主机客户端。

需要 `Microsoft.ElasticSan/elasticSans/volumegroups/delete`，卷／快照和私有连接的列表／读取权限，已审核前置步骤的删除权限，以及 SAN、资源组读取和管理锁列表权限。子资源身份或配置变化、新成员、保护设置、锁及保留策略变化都会阻止删除。活动与保留卷组列表加上自身读取，可区分永久不存在与同 ID 软删除。签名结果支持重启恢复，保留卷组会以原 ID 重新被发现。只有确认卷组原生终态后，缺失的快照集合才会改用每个已知快照的自身读取核验；卷的两类列表必须仍可读取，以确认保留身份。卷组成员记录过期时，需要重新扫描卷组后再清理。

完成成员审查后，可清理 SAN：卷组作为前置删除步骤，其卷和快照保持各自的依赖顺序；私有终结点连接必须独立选中。SAN 驱动在调用 `Microsoft.ElasticSan/elasticSans/delete` 前确认这些资源已移除，并在 SAN 消失后再次核对原生集合及每个已知子资源的自身读取。仅父资源或集合返回 404 不能完成任务。签名 Location 回执支持重启恢复，避免重复发送 DELETE。

需要 SAN 删除、卷组／卷／快照／私有连接列表及读取、已审查子步骤的删除权限，以及资源组和管理锁读取权限。同名重建、可写配置变化、保护标签、锁和新增子资源都会阻止删除；随子资源删除而变化的原生只读容量计数不会改变已审查配置。SAN 成员记录过期时，计划会阻塞，需要完整补扫相关资源。
...
完整扫描 SQL／Spark 池、代码资产和 Spark 任务记录后，可清理 Synapse 工作区。请核对完整影响列表：删除将移除 SQL 池、计算引擎、Notebook、作业定义、Pipeline 和工作区元数据，并中断工作区内的任务；关联的 Data Lake 存储将保留。保留任一工作区成员会阻止此操作；仅选择子资源不会自动选择工作区删除。

需要工作区删除权限、工作区和资源组读取、管理锁列表、SQL／Spark 池列表和读取、SQL 复制链接列表和读取，以及 Notebook、作业定义、Pipeline、批任务和会话的 Synapse 数据面列表和读取权限。新增成员、配置变化、保护设置或读取不完整时，需要重新审查。清理回执可跨重启恢复，并独立确认工作区和池已不存在；仅操作回调缺失或过期不能完成任务。单独删除代码资产仍未开放。参见[微软工作区删除范围说明](https://learn.microsoft.com/en-us/azure/synapse-analytics/quickstart-create-workspace-cli)。
...
Synapse 备份盘点现已包括可恢复的已删除 SQL 池和 SQL 池还原点。Azure 提供相应字段时，会记录创建／删除时间、最早恢复时间、还原点类型与标签，以及服务规格。这些记录与活动池分开保存；图中的关联表示查询它们所用的工作区或池，不授予级联删除权限。
...
符合条件的卷支持审阅后删除，范围包含卷内快照、子卷和配额规则。
请先停止应用，并从所有主机卸载此卷。备份库内的备份、容量池及帐户将保留。
...
扫描备份策略、快照策略和备份保管库时，还会读取卷的当前绑定关系。需要帐户内容量池及卷的列表／读取权限，
快照策略还需要关联卷列表权限。策略暂停或执行已禁用不代表绑定已解除；备份中保存的历史策略 ID 不代表当前绑定。
读取不可用时扫描失败并保留已有记录，原生策略索引不完整时保持未解决状态。
这些依赖观测不会授权删除卷。


快照策略支持在保留关联卷的情况下清理。计划会将卷及其已有快照、子卷和配额规则列为保留项；
执行时逐一解除已审阅的快照策略绑定，确认绑定消失后再删除策略。由此策略安排的后续快照将停止，
卷内数据、已有快照及备份保管库内的备份将保留。单独选择卷仍使用卷清理流程，不会同时选择快照策略。

除绑定发现所需读取权限外，还需要卷更新及快照策略删除权限。新增关联卷、绑定改为其他策略、卷配置或原生标识变化、
保护设置及父资源不可读取均会阻止修改。执行会保存每次更新和删除回执以支持重启恢复；回调完成后，
若卷仍显示旧绑定或策略仍存在，就不会确认清理完成。快照策略没有原生 UUID；缺少创建元数据时，
无法区分同名且配置完全相同的重建资源。参见[快照策略删除要求](https://learn.microsoft.com/en-us/azure/azure-netapp-files/snapshots-manage-policy#delete-a-snapshot-policy)。

备份策略也支持在保留卷和已有备份的情况下清理。执行会等待备份传输空闲，暂停已审阅卷上的策略执行，
确认暂停后再次等待传输完成，再单独解除备份策略绑定。所有已审阅绑定消失后才删除策略。
后续策略备份将停止；备份保管库、已有备份、快照、子卷、配额规则和卷内数据均保留。单独清理卷仍需另行选择。

除绑定发现的读取权限外，还需要卷更新、最新备份状态读取和备份策略删除权限。重新扫描必须取得原生策略 UUID
及完整关联卷索引。传输状态未知、读取不可用、策略执行被重新启用、新增关联卷或资源标识／配置变化均会阻止继续修改。
每次更新被接受后均会分别保存回执，以支持重启恢复；回调完成后仍需独立读取资源确认结果。
参见[备份策略管理](https://learn.microsoft.com/en-us/azure/azure-netapp-files/backup-manage-policies)。

备份保管库发现还需要库内备份的列表／读取权限。扫描会独立于卷的当前绑定记录完整原生备份清单，
包括源卷已删除的备份。此前已观察到但未出现在列表中的备份仍会按 ID 读取，只有备份自身不存在时
才解除该关联。读取失败或成员变化会使扫描失败并保留先前观测；备份图记录缺失或过期时需要刷新。

备份保管库清理会停止已审阅卷的计划备份，分别解除备份策略绑定，并永久删除库内所有已审阅备份。
逐一确认备份已不存在后，才解除保管库绑定并删除空保管库。卷及其数据、快照、子卷、配额规则和全局备份策略均保留。
删除最后一个备份还会移除后续增量备份的参考点；保留库内任一备份都会阻止保管库清理。

除完整发现和保护检查所需读取权限外，还需要卷更新、最新备份状态读取、备份策略读取、备份删除和保管库删除权限。
每次操作被接受后均会保存回执以支持重启恢复；仅回调完成不足以确认解绑或删除。
新增备份或关联卷、标识／配置变化、保护状态、未知传输状态及读取失败都会阻止继续修改。
清理途中出现的新备份需要重新审阅，即使计划备份已经停止。参见[保管库管理](https://learn.microsoft.com/en-us/azure/azure-netapp-files/backup-vault-manage)。

此流程已有离线契约、故障注入及重启测试，卷 PATCH 解绑尚未在真实 Azure 环境验收。
备份保管库没有原生 UUID；缺少创建元数据时，无法区分同名且配置完全相同的重建资源。

卷组发现会审查卷组详情中的卷 ID 及数量，再独立读取成员卷和帐户内完整的容量池／卷索引，
核对卷的当前组名；如果卷组内嵌详情提供文件系统 UUID，还必须与卷自身读取结果一致。
此前已观察到但被索引遗漏的成员仍会按 ID 读取，只有确认关联改变或卷自身不存在后才移除成员关系。
卷组扫描不会删除或核销卷资源记录。图记录缺失时需要刷新；读取失败或扫描期间发生变化时保留先前观测。
除卷组读取权限外，还需要帐户内容量池和卷的列表／读取权限。

卷组清理仍未实现。Azure 要求先删除全部成员卷，并在删除卷组时自动删除相关网络接口；
这些接口影响仍需审查后才能启用清理。参见[应用程序卷组删除](https://learn.microsoft.com/en-us/azure/azure-netapp-files/application-volume-group-delete)。

卷带有网络同级集合 ID 时，还需要原生网络同级集合查询权限
（`Microsoft.NetApp/locations/queryNetworkSiblingSet/action`）及返回的每个卷的读取权限。
只读 POST 查询标识共享主挂载 IP 的卷；扫描通过重复独立读取，核对返回的子网／集合标识、卷 ID、UUID 和配置。
如果卷提供挂载目标，还会核对 IP；可选挂载目标缺省时，仍以原生查询确认成员关系。

网络迁移或原生 UUID 暂缺时，卷仍可展示，但不允许清理。读取失败、新增关联卷、状态变化或活跃卷被遗漏时需要重新审阅。
清理途中，只有被移除的已审阅关联卷自身逐一返回 404，才接受集合缩小，以支持先前的卷清理步骤完成。
已有卷观测需要刷新以取得网络审查。网络 IP 不是 NIC 资源 ID；接口归属、保护和删除影响验证仍待实现。


网络接口的 `hostedWorkloads` 非空时，会阻止直接清理，以及会删除或改写该接口的
VM 级联清理。格式异常的工作负载信息同样受保护；字段缺失、为 null 或为空数组
本身不构成工作负载关联。清理时会重新读取接口，因此扫描后新增的工作负载也会
阻止执行。这些信息不会授予任何控制器删除 NIC 的权限。NetApp 卷组的接口归属及
自动删除仍需补齐独立的生命周期审查实现。


卷组扫描现在通过订阅范围内完整的 NIC 列表及接口自身读取记录关联信息，将 sibling-set
主 IP 与卷提供的全部挂载地址结合，并分两轮核对匹配的子网/IP、接口原生 GUID 和
关联工作负载 ID。需授予连接订阅范围内的 `Microsoft.Network/networkInterfaces/read`
权限。列表遗漏的已观测接口仍会单独读取；读取不可用或前后变化时，扫描失败并保留
上一次审查结果。

挂载信息或接口匹配缺失、原生 GUID 缺失、未知或共享工作负载及接口过渡状态都会使
关联审查不完整。只保存符合规范的卷 ID，不保存任意工作负载字段或 NIC 私密配置。
关联不等于卷组独占所有权，也不授权删除；接口生命周期影响尚未实现，卷组继续受保护。
旧的四字段卷组审查需要重新扫描。
