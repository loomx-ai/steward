...
卷组盘点还会读取活动／保留卷、快照以及 SAN 私有终结点连接，完整核验成员需要这些子资源的列表／读取权限。核验通过的卷组显示成员数量；无法解析卷组映射的私有连接仍作为可能的依赖保留。签名历史会恢复列表遗漏的已知成员。保留卷组只保留历史成员信息，不将其显示为当前数量；创建中的卷尚无 GUID 时，成员信息也保持未核验。如果保留卷组的快照列表返回 404，仍可发现的卷会继续显示，但禁止清理；权限失败仍会使扫描失败。
