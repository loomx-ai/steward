...
订阅级设置列表并不包含每个资源上的设置。Steward 结合原生子资源 API 和已保存的设置 ID 补充发现，因此源资源被删除后，已知设置仍能保留在盘点中。未知源类型的设置会先标为受保护，等待支持其原生身份校验；从未入库且不在已发现源范围内的孤立设置，没有可用的订阅级完整索引。列表或详情读取失败会阻止收敛或清理；源资源或资源组消失不能证明设置已删除。清理须等待设置自身的原生 GET 确认不存在。原生 DELETE 不支持版本条件参数，删除前的检查无法完全排除同时发生的外部修改。
