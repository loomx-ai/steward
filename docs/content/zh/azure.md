...
托管组清理会检查每个已审查且将被删除的 ARM 成员的传入引用。经验证的同组 Monitor 成员可以随控制器级联删除，外部引用仍会阻止清理。检查会重新核对私有配置、资源组身份和接收目标解析结果；控制器与资源组消失后，仍会读取已知残留成员。对资源组清单遗漏成员的完整原生发现，以及直接引用控制器的级联场景仍待接入。
