...
The example uses AWS sample resources: two instances, `api-01` and `api-02`, share an application subnet. A load balancer named `public-gateway` is also selected for review. These are illustrative records, not resources Steward creates in your account.
...
In the example, removing `api-01` does not establish that its shared subnet or security group is unused. Open the other instance and verify whether its owner still needs it. Check provider-side state when the graph is incomplete or stale.
...
| --- | --- |
| `api-02` must remain | Remove the shared subnet from the targets and review the updated plan. |
